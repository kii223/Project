...
urls = ['https://cn.tripadvisor.com/Attractions-g60763-Activities-c47-oa{}-New_York_City_New_York.html#ATTRACTION_LIST'.format(str(i)) for i in range(30,300,30)]
...
def getLocation(url):
    ...
    imgs = soup.select('#ATTR_ENTRY_ > div.attraction_clarity_cell > div > div > div.photo_booking.non_generic > a > img')
    tags = soup.select('#ATTR_ENTRY_ > div.attraction_clarity_cell > div > div > div.listing_info > div.tag_line > div')

    for image,title,tag in zip(imgs,titles,tags):
        data = {
        'title':title.get_text(),
        'tags':list(filter(lambda x:x != ',',list(tag.stripped_strings))),
        'image':image.get('src')
        }
        info.append(data)
    for i in info:
